...
export default function MenuTree() {
    ...
    const fetchMenus = async () => {
      try {
        ...
        const formatted = res.data.payload.map((m) => ({
          id: m.menuId,
          parent: m.menuParent === 0 ? 0 : m.menuParent,
          text: m.menuName,
          droppable: true,
        }));
  ...
  const handleDrop = async (newTree) => {
    ...
    const payload = newTree.map((n, index) => ({
      menuId: n.id,
      menuParent: n.parent === 0 ? 0 : n.parent,
      menuOrder: index + 1,
    }));
    ...
    >
      ...
      <Tree
        ...
        render={(node, { depth, isOpen, onToggle }) => (
          ...
          >
            {node.droppable && (
              <span onClick={onToggle} style={{ marginRight: "8px", userSelect: "none" }}>
                {isOpen ? "▼" : "▶"}
              </span>
            )}
            {node.text}
